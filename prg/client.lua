...
function SetFuel(vehicle, fuel)
    if type(fuel) == 'number' and fuel >= 0 and fuel <= 100 then
        ...
        if Config.ElectricVehicles[vehname] then
            ServerCallback('sna-fuel:server:GetTimeInGarage', function(time)
                if time then
...
CreateThread(function() -- Set target for pumps and blips
    ...
    Wait(100)
    AddTargetModel(FuelPumps, {{
        event = "sna-fuel:PickupPump",
        icon = "fas fa-gas-pump",
        label = Lang:t("info.pickup_pump"),
        entity = entity
    }, {
        event = "sna-fuel:BuyJerrican",
...
RegisterNetEvent("sna-fuel:BuyJerrican", function(data)
    local ped = PlayerPedId()
    local currentCash = GetPlayerMoney()
    if not HasPedGotWeapon(ped, 883325847) then
        if currentCash >= Config.JerryCanCost then
            TriggerServerEvent('sna-fuel:server:Pay', Config.JerryCanCost)
            ShowNotification(Lang:t("message.jerrican_bought"), "success")
            TriggerServerEvent('sna-fuel:server:GiveJerrican')
        end
    else
        local refillCost = Round(Config.JerryCanCapacity * Config.LiterPrice * (1 - GetAmmoInPedWeapon(ped, 883325847) / 4500))
        if refillCost > 0 then
            if currentCash >= refillCost then
                TriggerServerEvent('sna-fuel:server:Pay', refillCost)
                ShowNotification(Lang:t("message.jerrican_refilled"), "success")
                SetPedAmmo(ped, 883325847, 100)
                UpdateWeaponAmmo(100)
            else
                ShowNotification(Lang:t("message.no_money"), "error")
            end
        else
            ShowNotification(Lang:t("message.jerrican_full"), "error")
        end
    end
end)
...
RegisterNetEvent("sna-fuel:PickupPump", function(data)
    if CurrentPump then
        TriggerServerEvent('sna-fuel:server:DetachRope', PlayerPedId())
        ShowHelpNotification(nil)
    else
        ...
        TriggerServerEvent('sna-fuel:server:AttachRope', netIdProp, pumpcoords, GetEntityModel(CurrentPump))
    
        IsMounted = false
        DetectPetrolCap(Config.PumpModels[GetEntityModel(CurrentPump)].electric)
        ShowHelpNotification(Lang:t('info.info_pump'))
    end
end)

RegisterNetEvent("sna-fuel:RefuelVehicle", function(ped, vehicle)
    local startingfuel = DecorGetFloat(vehicle, Config.FuelDecor)
    local startingCash = GetPlayerMoney()
    local vehname = GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)):lower()
    local tank
    if Config.TankSizes[vehname] then
    ...
    while IsFueling or CurrentCost ~= 0 do
        ...
        if currentFuel <= tank then
            if CurrentPump == "can" then
                ...
                ShowHelpNotification(Lang:t('info.jerrican_refilling', Round(currentFuel - startingfuel, 1)))
            else
                if not Config.PumpModels[GetEntityModel(CurrentPump)].electric then
                    CurrentCost = CurrentCost + (Config.LiterPrice * fuelToAdd)
                    ShowHelpNotification(Lang:t('info.refilling', Round(currentFuel - startingfuel, 1), math.floor(CurrentCost)))
                else
                    CurrentCost = CurrentCost + (Config.KwPrice * fuelToAdd)
                    ShowHelpNotification(Lang:t('info.recharging', Round(currentFuel - startingfuel, 1), math.floor(CurrentCost)))
                end
        ...
        if IsFueling then
            ...
            if CurrentCost ~= 0 then
                ...
                TriggerServerEvent('sna-fuel:server:Pay', math.floor(CurrentCost))
                CurrentCost = 0
            end
        end
    end
            ...
            if IsControlPressed(0, Config.ActionKey) and CurrentCapPos then
                ...
                if #(GetEntityCoords(playerPed) - CurrentCapPos) < 3.0 then -- Mount/Dismount pump
                    if not IsMounted then
                        ...
                        IsFueling = true
                        CurrentCost = 0
                        TriggerEvent("sna-fuel:RefuelVehicle", playerPed, CurrentVehicle)
                    else
                        -- Dismount
                        IsMounted = false
                        IsFueling = false
    ...
    while true do
        ...
        if IsPedInAnyVehicle(ped, false) then
            ...
            if GetSelectedPedWeapon(ped) == 883325847 then
                if GetAmmoInPedWeapon(ped, 883325847) >= 1 and not CurrentCapPos then
                    CurrentPump = "can"
                    DetectPetrolCap(nil)
                end
            else
        ...
        if CurrentPump and not IsMounted and CurrentPump ~= "can" then       --Check max distance
            if #(GetEntityCoords(ped) - GetEntityCoords(CurrentPump)) >= Config.RopeMaxLength then
                TriggerEvent('sna-fuel:PickupPump')
                ShowNotification(Lang:t("message.to_far_away"), "error")
            end
        end
...
RegisterNetEvent("sna-fuel:SetFuel", function(fuel)
    local ped = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(ped, false)
    if GetPedInVehicleSeat(vehicle, -1) == ped then
        ApplyFuel(vehicle, fuel)
    else
        ShowNotification(Lang:t("message.must_be_driver"), "primary")
    end
end)

RegisterNetEvent("sna-fuel:client:AttachRope", function(netIdProp, posPump, model, citizenid)
...
RegisterNetEvent("sna-fuel:client:DetachRope", function(citizenid, src)
